...
public class ToolWeb {
	...
	public static void addCookie(HttpServletResponse response, String domain, String path, boolean isHttpOnly,
		...
		Cookie cookie = new Cookie(name, ToolString.urlEncode(value));
    ...
    public static String getAuthReqCtx(HttpServletRequest request) {

        String strReqestUri = request.getRequestURI();
        String strQueryString = StringUtils.EMPTY;

        if (StringUtils.isEmpty(strReqestUri)) {
            return StringUtils.EMPTY;
        }

        try {
            if (StringUtils.isNotEmpty(request.getQueryString())) {
                strQueryString = AppConstants.QUERY_START_STRING
                        + URLDecoder.decode(request.getQueryString(), "UTF-8");
            }
        } catch (Exception e) {
        }

        return strReqestUri.substring(strReqestUri
                .lastIndexOf(AppConstants.API_PREFIX))
                + strQueryString;
    }
